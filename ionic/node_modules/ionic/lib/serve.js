...
class ServeRunner {
    ...
    async run(options) {
        ...
        if (options.open) {
            ...
            await open_1.open(openURL, { app: options.browser });
            this.e.log.info(`Browser window opened to ${color_1.strong(openURL)}!`);
            this.e.log.nl();
        }
...
class ServeCLI extends events_1.EventEmitter {
    ...
    async spawn(options) {
        ...
        const p = await this.e.shell.spawn(this.resolvedProgram, args, { stdio: 'pipe', cwd: this.e.project.directory, env: utils_process_1.createProcessEnv(envVars) });
