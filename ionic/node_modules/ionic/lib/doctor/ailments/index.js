...
class IonicCLIInstalledLocally extends base_1.Ailment {
    ...
    async getTreatmentSteps() {
        const [manager, ...managerArgs] = await npm_1.pkgManagerArgs(this.config.get('npmClient'), { command: 'uninstall', pkg: 'ionic' });
        return [
            {
                message: `Run: ${color_1.input(manager + ' ' + managerArgs.join(' '))}`,
                treat: async () => {
...
class CordovaPlatformsCommitted extends base_1.Ailment {
    ...
    async getMessage() {
        return (`Cordova ${color_1.strong('platforms/')} directory is committed to git.\n` +
            ...
            `${chalk_1.default.yellow(`${color_1.strong('WARNING')}: Attempting to fix this could be dangerous. Only proceed if you're sure you haven't made manual modifications to these files.`)}\n`).trim();
    }
    async detected() {
        if (!(await git_1.isRepoInitialized(this.project.directory))) {
            return false;
