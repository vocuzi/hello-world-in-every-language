    ...
    const getInfo = async () => {
        ...
        const info = [
            ...
            { group: 'system', key: 'OS', value: os },
            {
                group: 'utility',
                key: 'native-run',
                value: nativeRun ? (`${nativeRun} ${nativeRunUpdate ? `(update available: ${latestNativeRun ? color_1.success(latestNativeRun.version) : '???'})` : ''}`) : 'not installed',
            },
            {
                group: 'utility',
                key: 'cordova-res',
                value: cordovaRes ? (`${cordovaRes} ${cordovaResUpdate ? `(update available: ${latestCordovaRes ? color_1.success(latestCordovaRes.version) : '???'})` : ''}`) : 'not installed',
            },
        ];
