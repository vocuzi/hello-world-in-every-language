...
Use the ${color_1.input('--type')} option to start projects using older versions of Ionic. For example, you can start an Ionic 3 project with ${color_1.input('--type=ionic-angular')}. Use ${color_1.input('--list')} to see all project types and templates.
    ...
    async run(inputs, options, runinfo) {
        ...
        if (this.project && this.project.details.context === 'multiapp' && !this.schema.cloned) {
            ...
            await utils_fs_1.unlink(path.resolve(projectDir, 'ionic.config.json'));
    ...
    async showNextSteps(projectDir, cloned, linkConfirmed) {
        const steps = [
            `Go to your ${cloned ? 'cloned' : 'newly created'} project: ${color_1.input(`cd ${format_1.prettyPath(projectDir)}`)}`,
            `Run ${color_1.input('ionic serve')} within the app directory to see your app`,
            `Build features and components: ${color_1.strong('https://ion.link/scaffolding-docs')}`,
            `Get Ionic DevApp for easy device testing: ${color_1.strong('https://ion.link/devapp')}`,
        ];
        if (linkConfirmed) {
            steps.push(`Push your code to Ionic Appflow to perform real-time updates, and more: ${color_1.input('git push ionic master')}`);
        }
        this.env.log.info(`${color_1.strong('Next Steps')}:\n${steps.map(s => `- ${s}`).join('\n')}`);
    }
}
